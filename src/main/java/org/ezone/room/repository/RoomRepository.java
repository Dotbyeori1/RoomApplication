...
public interface RoomRepository extends JpaRepository<RoomEntity,Long> {
    ...
    @Query("select room, r from RoomEntity room left join ReservationEntity as r " +
            "on room = r.room_id and (r.StartDate < :end_date and r.EndDate > :start_date) " +
            "and r.state = true and room.operating = true " +
            "where room.rno = :rno group by room.rno")
    ...
    @Query("select room, img.Imgfile, r.rvno from RoomEntity room left join ReservationEntity as r " +
            ...
            "where room.operating = true and room.accommodationEntity.ano = :ano group by room.rno")
    ...
    @Query("select rv.rvno, rv.state, rv.regTime,rv.StartDate,rv.EndDate, reserver, rv.message from Member m" +
            " join Accomodation a on m.id = a.member.id" +
            " join RoomEntity r on r.accommodationEntity = a" +
            " join ReservationEntity rv on rv.room_id = r" +
            " join Member reserver on reserver.id = rv.member_id.id" +
            " where m.email = :email")
    List<Object[]> findSalesByEmail(@Param("email") String email);
}
