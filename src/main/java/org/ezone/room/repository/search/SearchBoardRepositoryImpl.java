    ...
    public Page<Object[]> searchPageAccommodation(String[] type, String keyword, String category, String region,
        ...
        JPQLQuery<Tuple> query = from(accommodation)
                ...
                .join(roomEntity).on(accommodation.eq(roomEntity.accommodationEntity)
                        .and(roomEntity.operating.isTrue()))
                ...
                .where(roomEntity.accommodationEntity.eq(accommodation)
                        .and(reservationEntity.rvno.isNull().or(reservationEntity.room_id.isNull())))
                .groupBy(accommodation);


        // 키워드 검색 하는 곳 (검색 파라미터 삽입하는 곳)
        if (type != null || region != null || category != null || keyword != null
        || inputedMinPrice != null || inputedMaxPrice != null ||  tag != null ) {
            BooleanBuilder conditionBuilder = new BooleanBuilder();
            ...
            if (tag != null) {
                int selectedTagCount = Integer.bitCount(tag);
                conditionBuilder.and(Expressions.booleanTemplate("bit_count(bitand({0}, {1})) = {2}", roomEntity.tag, tag, selectedTagCount));
            }
